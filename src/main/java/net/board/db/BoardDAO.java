...
public class BoardDAO {
	...
	public int getListCount() {
		...
		int x = 0;
		try (Connection con = ds.getConnection();  // 데이터베이스 연결
				PreparedStatement pstmt = con.prepareStatement(sql);) { 

			try (ResultSet rs = pstmt.executeQuery()){   /* 쿼리실행 후 그 값을 rs에 저장)*/
				if (rs.next()) {  //값이 있으면 true 없으면 
					x = rs.getInt(1);
				}
			...
			ex.printStackTrace();// getListCount() end
		}
		return x;
	}
...
// 글 목록보기    	 
	public List<BoardBean> getBoardList(int page, int limit) {
		...
		// 조건절에 맞는 rnum의 범위만큼 가져오는 쿼ㅣ문입니다.
		...
		int endrow = startrow + limit - 1; // 읽을 마지막 페이지 row번호 (10,20,30 ...
...
return result;
		...
		public int boardReply(BoardBean board) {
			...
			/*
			 답변을 달 원문 글 그룹번호입니다.
			 답변을 달게 되면 답변 글은 이 번호와 같은 관련글 번호를 갖게 되면서 같은 그룹에 속하게됩니다.
			...
			*/
			...
			/*
			 답글의 깊이를 의미합니다.
			 원문에 대한 답글이 출력될 때 한 번 들여쓰기가 처리가 되고 답글에 대한 답글은 들여쓰기가 두번 처리되게 합니다.
			...
			*/
			...
			try (Connection con = ds.getConnection();){
				...
				//트랜잭션을 이용하기 위해서 setAutoComit을 false 로 설정합니다.
		...
		public void reply_update(Connection con, int re_ref, int re_seq)throws Exception {
			...
			//달린 댓글들의 BOARD_RE_SEQ 값을 1씩 증가시킵니다.
		...
		public int reply_insert(Connection con, BoardBean board, 
		...
		try (ResultSet rs = pstmt.executeQuery();){ //2
			if(rs.next()) {
				try(PreparedStatement pstmt2 = con.prepareStatement(board_delete_sql)){ //3
					...
					if(count>=1)
						result_check = true;// 삭제가 안되는 경우에는 false 를 반환합니다.
				}//try 3
			}// if (rs.next()){
		}//try 2
	} catch (Exception ex) {
    	 System.out.println("boardDelecte() 에러 : " + ex);
    	 ex.printStackTrace();
     }
		return result_check;
}//boardDelete() 메서드 end
