...
public class CafeAdminAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		// 리스트 받아오기
		itemlist = idao.getItemList(page, limit);
		...
		if(state == null) { // ☆request로 담기
			...
			request.setAttribute("boardlist", itemlist);
			
			request.setAttribute("limit", limit);
			ActionForward forward = new ActionForward();
			forward.setRedirect(false); // forward 방식(주소 변경없이 값 그대로 들고 가기 위해)
			
			forward.setPath("cafe/itemlist.jsp");
			return forward;
